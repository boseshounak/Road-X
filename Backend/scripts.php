<?php
	...
	if(strcmp("signup",$type)==0){
		...
		$prob=$_POST["prob"];
		$date=$_POST["date"];
		$que="INSERT INTO FindAll2(email,vendor,vechicle,prob,date) VALUES('$user_id','$vendor','$vechicle','$prob','$date')";
		$query=mysqli_query($con,$que);
		if($query){
			$to      = $user_id;
			$subject = 'Booking Confirmation';
			$message = 'Your Booking of the mechanic has been confirmmed by'.$vendor.' on '.$date.' for the following problem '.$prob.
